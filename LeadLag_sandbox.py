...
summed_data = np.sum(loaded_daily_data[:1518, :, :], axis=0)
masked_array = np.ma.masked_array(summed_data, mask=np.eye(498, dtype=bool))
indices_flattened = np.argpartition(masked_array.ravel(), -20)[-20:]
...
twoyears_daily_data = loaded_daily_data[-1000:, :, :]

out_degrees = np.sum(summed_data, axis=1)

lagger_out_degrees = {stock: out_degrees[stock_names.index(stock)] for stock in leader_lagger_dict.values()}

sorted_laggers = sorted(lagger_out_degrees.items(), key=lambda x: x[1])

lowest_out_degree_laggers = [lagger[0] for lagger in sorted_laggers[:5]]

new_leader_lagger_dict = {leader: lagger for leader, lagger in leader_lagger_dict.items() if lagger in lowest_out_degree_laggers}

stocks_to_include = set(new_leader_lagger_dict.keys()).union(set(new_leader_lagger_dict.values()))
filtered_stock_data = stockData.loc[:, stocks_to_include].tail(1000)

initial_investment = 500000
n_laggers = len(new_leader_lagger_dict.values())
for stock in stocks_to_include:
    if stock in filtered_stock_data.columns:
        price = filtered_stock_data[stock].iloc[0]
        initial_investment += price
investment_per_stock = initial_investment / len(stocks_to_include)
portfolio = {stock: 0 for stock in new_leader_lagger_dict.values()}
...
#     highest_leader_prices = {leader: 0 for leader in new_leader_lagger_dict.keys()}
#     for leader, lagger in new_leader_lagger_dict.items():
#         if leader not in filtered_stock_data.columns or lagger not in filtered_stock_data.columns:
#             continue
...
#                 # print(f"Day {i}: Buying {lagger} at {round(price, 2)}")
...
#                         # print(f"Day {j}: Selling {lagger} at {round(price, 2)}")
...
for day in range(n_days):
    ...
    for leader, lagger in new_leader_lagger_dict.items():
        ...
        current_leader_price = filtered_stock_data[leader].iloc[day]
        highest_leader_prices[leader] = max(highest_leader_prices[leader], current_leader_price)
        if current_leader_price >= 1.02 * filtered_stock_data[leader].iloc[day - 1]:
            price = filtered_stock_data[lagger].iloc[day]
            # print(f"Day {day}: Buying {lagger} at {round(price, 2)}")
            buy_stock(portfolio, lagger, investment_per_stock, price)

            for j in range(day, n_days):
                current_leader_price = filtered_stock_data[leader].iloc[j]
                highest_leader_prices[leader] = max(highest_leader_prices[leader], current_leader_price)
                if current_leader_price <= (1 - trailing_stop_percentage) * highest_leader_prices[leader]:
                    price = filtered_stock_data[lagger].iloc[j]
...
ax1.plot(range(0, n_days), portfolio_values, color='blue')
ax1.set_xlabel("Days")
ax1.set_ylabel("Portfolio Value", color='blue')
ax1.tick_params(axis='y', labelcolor='blue')
ax2 = ax1.twinx()
ax2.plot(range(0, n_days), [100 * (value - initial_value) / initial_value for value in portfolio_values], color='red')
